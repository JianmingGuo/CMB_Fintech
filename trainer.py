...
def get_data():
    ...
    print('--------')
    infoDf = pd.read_csv('../reg_stage1_0502/train.csv')
    # 含有thresh个非0的值
...
def trainer(dtrain, dval, useTrainCV=False):
    params = {
        'booster': 'gbtree',
        'objective': 'binary:logistic',  # 多分类的问题          
        'gamma': 0.2,                  # 用于控制是否后剪枝的参数,越大越保守，一般0.1、0.2这样子。
    ...
    saved_path = '/work/reg_stage1_0505/reg_0505.json'
...
# if __name__ == '__main__':
    # ParamSearch()
if __name__ == '__main__':
    dtrain, dval = get_data()
    clf = trainer(dtrain, dval, useTrainCV=True)
